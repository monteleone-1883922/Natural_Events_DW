...
FOR (tra:Trace)
ON (tra.id);
CREATE RANGE INDEX trace_tornado_id_index
FOR (tra:Trace)
ON (tra.tornado_id);

MATCH (tornado)-[r:TRACE]->(trace:Trace)
WITH tornado, r.order_idx as original_order, collect({rel: r, trace: trace}) as duplicates
WHERE size(duplicates) > 1
UNWIND range(0, size(duplicates)-1) as idx
WITH original_order, duplicates[idx] as dup, idx
SET dup.rel.order_idx = original_order + (idx * 0.1)
RETURN count(*) as updated_relations;
...
WITH row,
     ...
     toInteger(row.counter) as counter
...
FOREACH (_ IN CASE WHEN f1 > 0 AND ns = 1 AND ct1 IS NOT NULL THEN [1] ELSE [] END |
  CREATE (t)-[:AFFECTS {order_idx: counter}]->(ct1)
)
FOREACH (_ IN CASE WHEN f1 > 0 AND ns = 1 AND ct1 IS NULL AND ict1 IS NOT NULL THEN [1] ELSE [] END |
  CREATE (t)-[:AFFECTS {order_idx: counter}]->(ict1)
)
// F2 ------------------------------------------
WITH t, row, f1, f2, f3, f4, s, ns, sn, sg, CASE WHEN ct1 IS NOT NULL OR ict1 IS NOT NULL THEN counter+1 ELSE counter END as i
