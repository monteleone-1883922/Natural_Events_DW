...
FOREACH (_ IN CASE WHEN f1 > 0 THEN [1] ELSE [] END |
  MERGE (tr)-[:AFFECTS_COUNTY {order_idx: 1}]->(:County {fips_code: f1})
)
FOREACH (_ IN CASE WHEN f2 > 0 THEN [1] ELSE [] END |
  MERGE (tr)-[:AFFECTS_COUNTY {order_idx: 2}]->(:County {fips_code: f2})
)
FOREACH (_ IN CASE WHEN f3 > 0 THEN [1] ELSE [] END |
  MERGE (tr)-[:AFFECTS_COUNTY {order_idx: 3}]->(:County {fips_code: f3})
)
FOREACH (_ IN CASE WHEN f4 > 0 THEN [1] ELSE [] END |
  MERGE (tr)-[:AFFECTS_COUNTY {order_idx: 4}]->(:County {fips_code: f4})
);